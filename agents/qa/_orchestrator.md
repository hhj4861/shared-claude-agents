...
E2E_테스트_요청:
  ...
  동작: /e2e-test 스킬 호출 (메인 에이전트에서 직접 실행)
  명령: Skill(skill="e2e-test")
  참고: step4-e2e-tester 에이전트는 archived됨

API_테스트_요청:
  키워드: [API, api, 백엔드 테스트]
  동작: /api-test 스킬 호출
  명령: Skill(skill="api-test")
```
...
| 키워드 | 실행 방법 | 비고 |
|--------|-----------|------|
| E2E, e2e, 브라우저, UI, 화면 테스트 | **Skill("e2e-test")** | 메인 에이전트에서 직접 |
| API, 백엔드 테스트 | **Skill("api-test")** | 메인 에이전트에서 직접 |
| 시나리오, QA 시나리오 | 아래 파이프라인 | 서브에이전트 사용 |
...
### Step 2: /e2e-test 스킬 호출

**⚠️ 메인 에이전트에서 직접 스킬을 호출하여 테스트 실행**
...
⚠️ 자동 진행 원칙:
  - 모든 서브에이전트(step1~step3.6)는 질문 없이 자동 진행
  - 실패 시 자동 건너뛰기 또는 기본값 사용
  - 치명적 오류만 보고

ALWAYS_DO:
  - ✅ E2E 요청 → Skill("e2e-test") 호출
  - ✅ API 요청 → Skill("api-test") 호출
  - ✅ 시나리오 요청 → 아래 파이프라인 실행
```
...
└─────────────────────────────────────────────────────────────────┘
         ...
         ▼
┌─────────────────────────────────────────────────────────────────┐
│  STEP 4.6: 시나리오 보완 (에이전트) 🆕 [자동 실행]               │
│  Task(subagent_type: "step3.6-scenario-refiner",                │
│       prompt: config_path)                                       │
│  → step3.5 리뷰 피드백 반영 (Opus 모델)                          │
│  → 누락 시나리오 추가, 중복/불필요 테스트 제거                    │
│  → scenarios/*.md 최종 보완                                      │
│  → refinement-log.md 생성 (변경 내역)                           │
│  ⚠️ 선택적: 리뷰 파일 없으면 스킵 (step3.5 미실행 시)            │
└─────────────────────────────────────────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────────────────────────────────┐
...
5.5_시나리오_외부_검토 (자동 실행):
  명령: Task(subagent_type="step3.5-scenario-reviewer", prompt="{config_path} 기반으로 시나리오 검토 --auto")
  실행: step3 완료 후 자동 실행
  Gemini_없을_경우: 스킵 (동일 LLM 재검토는 무의미)
  입력: references/, analysis/, scenarios/ (전체 컨텍스트 전달)
  출력: docs/qa/latest/review/scenario-review.md

5.6_시나리오_보완 (자동 실행):
  명령: Task(subagent_type="step3.6-scenario-refiner", prompt="{config_path} 기반으로 시나리오 보완 --auto")
  실행: step3.5 완료 후 자동 실행 (리뷰 있을 때만)
  리뷰_없을_경우: 스킵 (step3 결과를 최종본으로 사용)
  입력: scenarios/, review/scenario-review.md
  출력: scenarios/*.md (최종 보완), refinement-log.md
...
테스트를 실행하려면:
- "API 테스트 실행해줘" → /api-test 스킬
- "E2E 테스트 실행해줘" → /e2e-test 스킬
```
...
step3.6-scenario-refiner:
  model: opus
  역할: 시나리오 보완 (리뷰 피드백 반영)
  특징: 리뷰 없으면 스킵, 누락 추가/중복 제거

# 테스트 실행 (스킬 기반)
/e2e-test:
  실행: 메인 에이전트에서 직접 실행
  역할: Playwright MCP로 E2E 테스트 실행
  파일: skills/e2e-test/SKILL.md

/api-test:
  실행: 메인 에이전트에서 직접 실행
  역할: API 테스트 실행
  파일: skills/api-test/SKILL.md
```
