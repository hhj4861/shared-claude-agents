...
`;
      ...
      case "e2e_update_result": {
        ...
        // 🔄 대시보드 API 동기화 (localhost:3847)
        const dashboardStatus = status === "pass" ? "passed" : status === "fail" ? "failed" : status === "pending" ? "pending_hold" : "skip";
        const dashboardMessage = error || `TC ${tc_id} ${dashboardStatus}`;

        try {
          const dashboardUrl = process.env.DASHBOARD_URL || "http://localhost:3847";
          const response = await fetch(`${dashboardUrl}/api/tc/complete`, {
            method: "POST",
          ...
          return {
            content: [{
              ...
              text: JSON.stringify({
                ...
                status,
                current_stats: stats,
                dashboard_synced: response.ok,
                dashboard_result: dashboardResult
              }, null, 2)
            }]
          };
        } catch (dashboardError) {
          // 대시보드 동기화 실패해도 상태 파일 업데이트는 성공으로 처리
          return {
            content: [{
              type: "text" as const,
              text: JSON.stringify({
                ...
                dashboard_error: String(dashboardError)
              }, null, 2)
            }]
          };
        }
